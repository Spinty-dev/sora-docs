...
### 2.6 Phase 4: Advanced Attack Engines
...
#### 2.6.5 StealthEngine (Anti-Detection)

**Проблема:** без ограничений SORA инъектирует фреймы с постоянным интервалом и от статичного MAC. Это детектируется любым пассивным WIDS/IDS — Kismet, Snort с WiFi-плагинами, Cisco WIPS. Признаки: идеально ровный Beacon interval, нереалистичный burst TX, MAC из неиспользуемого OUI.
...
| Профиль | Детектируется Kismet? | Производительность | Сценарий |
| :--- | :--- | :--- | :--- |
| `off` | Да (сразу) | Максимальная | Изолированная тестовая среда |
| `low` | Вероятно | ~95% | Среды без WIDS |
| `medium` | Возможно при долгой работе | ~60–70% | Большинство реальных пентестов |
| `high` | Маловероятно | ~15–25% | Среды с активным WIDS |
